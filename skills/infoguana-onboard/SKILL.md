...
8. **Write the `CLAUDE.md`** using exactly this shape. Do not add sections, do not restate infoguana-protocol rules, do not pad:
   ...
   - **Reading:** If a SessionStart preamble delivered `context` hits, read them before acting. If none arrived — Codex before you approve the hook's trust hash, and `codex exec`, which runs no hooks at all — call `context(project="<PROJECT NAME>")` yourself first. On any "how does X work / state of Y / explain Z" question, `search` first before reading source or dispatching Explore agents. Only touch source to verify a specific line or fill a clear gap.
   ...
   Full infoguana-protocol guidance (good/bad memory examples, when-to-save rules) arrives in the SessionStart preamble — no need to restate it here. If no preamble arrived, that guidance is in the protocol note the server serves; `context` returns it.
   ```
